 ...
 * `estanciaMinima` is that new período's own minimum-stay value (null = none set) — a separate concept
 * from the per-day ajustes_dia override the bulk-edit dialog's own "Estancia mínima" field writes.
...
export async function asignarTemporadaRango(
  ...
  const { data, error } = await pricingDb().rpc("asignar_temporada_rango", {
    ...
    p_estancia_minima: estanciaMinima,
