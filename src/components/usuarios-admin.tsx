...
function roleStyle(acceso_app: string | null) {
  return acceso_app && ACCESO_STYLE[acceso_app] ? ACCESO_STYLE[acceso_app] : NULL_STYLE;
}

async function sendInvite(email: string) {
  // Client-safe invitation: sends magic link, creates auth user if missing.
...
export function UsuariosAdmin() {
  const personalQ = useQuery({
    ...
    queryFn: async () => {
      const { data, error } = await supabase
        .from("personal")
        .select("id_persona,nombre,apellidos,codigo,mail,activo")
  ...
  const prQ = useQuery({
    queryKey: ["usuarios-personal-roles"],
    queryFn: async () => {
      const { data, error } = await supabase
        .from("personal_roles")
  ...
  return (
    <div className="space-y-4">
      <div className="flex items-center justify-between">
        <p className="text-sm text-muted-foreground">{personalQ.data?.length ?? 0} usuarios</p>
        ...
        <Table>
          ...
          <TableBody>
            ...
            {!personalQ.isLoading && (personalQ.data ?? []).length === 0 && (
              <TableRow><TableCell colSpan={7} className="text-center py-6 text-muted-foreground">Sin usuarios</TableCell></TableRow>
            )}
  ...
  return (
    <Dialog open onOpenChange={(o) => !o && onClose()}>
      <DialogContent className="max-w-xl max-h-[90vh] overflow-y-auto">
        <DialogHeader>
          <DialogTitle>{persona ? "Editar usuario" : "Nuevo usuario"}</DialogTitle>
          <DialogDescription className="sr-only">Datos del usuario</DialogDescription>
        </DialogHeader>
