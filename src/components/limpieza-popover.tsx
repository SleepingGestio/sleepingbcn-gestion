...
export function LimpiezaPopover({ open, loadKey, onOpenChange, apt, fecha, existing, onSaved, readOnly = false }: Props) {
  ...
  const popoverDataQ = useQuery({
    ...
    queryFn: async () => {
      ...
      const kbData = await computeFreshKbData(supabase, apt, persisted, fecha);
      // kbData is non-null here: the "intermedia" case already returned above.
      const { fresh, stored, realCheckoutDate: checkoutDate, currentNumero, nextReservation: next, winMins } = kbData!;
      const isCurCancelada = kbData!.reason === "cancelada";
      ...
      const formBase = affected
        ...
        : {
            ...
            numero_reserva: currentNumero,
            tipo: "salida",
            hora_out_time: fresh.hora_out_time,
            hora_out_informed: fresh.hora_out_informed,
            hora_in_time: fresh.hora_in_time,
            hora_in_informed: fresh.hora_in_informed,
            prioritaria: winMins != null && winMins >= 0 && winMins < 150,
            sfc_montar: persisted.sfc_montar_manual ?? fresh.sfc_montar_auto,
            sfc_desmontar: persisted.sfc_desmontar_manual ?? fresh.sfc_desmontar_auto,
          };
      return {
        form: formBase,
  ...
  const kbChanges: KbChangeDiffEntry[] =
    showKbAlert && kbChange?.fresh && kbChange?.stored ? buildKbChanges(kbChange.fresh, kbChange.stored) : [];
  ...
  const applyFresh = async () => {
    ...
    try {
      ...
      const payload: any = {
        ...
        affected_by_kb_change: false,
        affected_reason: null,
        // Snapshot of what changed, so every screen can show a "resolved today"
