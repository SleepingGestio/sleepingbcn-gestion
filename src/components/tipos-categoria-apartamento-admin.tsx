...
type CategoriaApartamento = {
  id_categoria: number;
  nombre: string;
  activo: boolean | null;
};

export function CategoriasApartamentoAdmin() {
  const [createOpen, setCreateOpen] = useState(false);
  const [editing, setEditing] = useState<CategoriaApartamento | null>(null);

  const q = useQuery({
    queryKey: ["tipos-categoria-apartamento-all"],
    queryFn: async (): Promise<CategoriaApartamento[]> => {
      const { data, error } = await supabase
        .from("tipos_categoria_apartamento")
        .select("id_categoria,nombre,activo")
        .order("nombre");
      if (error) throw error;
      return (data ?? []) as CategoriaApartamento[];
    },
  });
    ...
    <Card className="p-4 space-y-4">
      <div className="flex items-center justify-between">
        <div className="text-sm text-muted-foreground">
          Gestiona las categorías de apartamento usadas para compartir tiempos estándar de limpieza.
        </div>
...
function CreateCategoriaModal({
  ...
  async function crear() {
    ...
    setBusy(true);
    const { error } = await supabase.from("tipos_categoria_apartamento").insert({ nombre: n, activo: true });
    setBusy(false);
  ...
  return (
    <Dialog open={open} onOpenChange={(v) => { if (!v) reset(); onOpenChange(v); }}>
      <DialogContent>
        <DialogHeader>
          <DialogTitle>Nueva categoría de apartamento</DialogTitle>
          <DialogDescription className="sr-only">Crear categoría de apartamento</DialogDescription>
        </DialogHeader>
        <div className="space-y-3">
          <div className="space-y-1.5">
            ...
            <Input
              ...
              placeholder="Ej. Apartamento, Habitación…"
            />
          </div>
        </div>
        <DialogFooter>
...
function EditCategoriaModal({
  ...
  categoria: CategoriaApartamento | null;
  onOpenChange: (v: boolean) => void;
  onSaved: () => void;
}) {
  const [nombre, setNombre] = useState("");
  ...
  return (
    <Dialog open={!!categoria} onOpenChange={onOpenChange}>
      <DialogContent>
        <DialogHeader>
          <DialogTitle>Editar categoría de apartamento</DialogTitle>
          <DialogDescription className="sr-only">Editar categoría de apartamento</DialogDescription>
        </DialogHeader>
        <div className="space-y-3">
          <div className="space-y-1.5">
            <Label>Nombre</Label>
